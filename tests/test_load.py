...
class MockDataset:
    def __init__(self, size=100, channels=1, height=28, width=28):
        self.size = size
        # Create mock data with the appropriate shape
        self.data = torch.rand((size, channels, height, width))  # Creating a tensor of shape [size, channels, height, width]
...
@pytest.fixture
def dataset_params(tmp_path) -> LoadDatasetParams:
...
@pytest.mark.parametrize("dataset_name, expected_function_name", [
...
def test_load_standard_dataset(dataset_params: LoadDatasetParams, dataset_name: DatasetNames) -> None:
    ...
    assert isinstance(image_size, ImageParams) and len(image_size.image_size) == 3, "Image size should be a ImageParams, tuple of length 3"
...
def test_load_dataset_function_call(mock_get_function, dataset_params: LoadDatasetParams) -> None:
    ...
    expected_params = DatasetParams(dataroot=dataset_params.dataroot, train=dataset_params.train, pytesting=dataset_params.pytesting)
...
def test_load_dataset_classes_range(dataset_params: LoadDatasetParams, dataset_name: DatasetNames) -> None:
    ...
    with pytest.raises(ValueError, match=r".*Invalid pos_class.*|.*Invalid neg_class.*"):
        load_dataset(LoadDatasetParams(
