...
def test_construct_classifier_ensemble_mean():
    params = ClassifierParams(type="ensemble:cnn:mean", img_size=(3, 64, 64), n_classes=10, nf=[[32, 16, 8], [32, 16, 8]])
    classifier = construct_classifier(params)
    assert isinstance(classifier, Ensemble)
    assert classifier.params.ensemble_type == EnsembleType.cnn  # Verify ensemble type
    assert classifier.params.output_method == OutputMethod.mean  # Verify output method


def test_construct_classifier_ensemble_linear():
    params = ClassifierParams(type="ensemble:cnn:linear", img_size=(3, 64, 64), n_classes=10, nf=[[32], [16]])
    classifier = construct_classifier(params)
    assert isinstance(classifier, Ensemble)
    assert classifier.params.ensemble_type == EnsembleType.cnn  # Verify ensemble type
    assert classifier.params.output_method == OutputMethod.linear  # Verify output method


def test_construct_classifier_ensemble_meta_learner():
    params = ClassifierParams(type="ensemble:cnn:meta-learner", img_size=(3, 64, 64), n_classes=10, nf=[[32], [16]])
    classifier = construct_classifier(params)
    assert isinstance(classifier, Ensemble)
    assert classifier.params.ensemble_type == EnsembleType.cnn  # Verify ensemble type
    assert classifier.params.output_method == OutputMethod.meta_learner  # Verify output method
