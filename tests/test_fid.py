...
@pytest.fixture
def fid_instance(fid_stats_file):
...
def test_initialization_success(fid_stats_file):
...
def test_finalize(fid_instance):
...
def test_reset(fid_instance):
    ...
    assert torch.equal(fid_instance.fid.fake_cov_sum, torch.zeros((fid_instance.dims, fid_instance.dims)).to(fid_instance.device))


def test_update_on_different_devices(fid_stats_file):
...
def test_finalize_with_mock(fid_instance):
...
def test_invalid_images_update(fid_instance):
