...
from src.models import ClassifierParams, PoolingParams, DeviceType, ClassifierType
...
def test_pool_out() -> None:
    ...
    assert pool_out(params) == 15, "Expected output size for 32x32 input with 2x2 pooling and dilation 2 should be 15x15"
...
def test_classifier_with_feature_maps(mock_data: Tensor) -> None:
    ...
    assert feature_maps[-1].shape == (1, 10), f"Expected last feature map to have shape (1, 10), but got {feature_maps[-1].shape}"
