...
class ClassifierParams(BaseModel):
    ...
    img_size: Tuple[int, int, int] = Field(
        ..., description="Tuple of three integers for image size"
    )
    n_classes: int = Field(...,
                           description="Number of classes (should be >= 2)")
    nf: Optional[Union[int, List[int], List[List[int]]]] = Field(
        None,
        description="Number of filters for cnn/mlp. Can be None for pretrained ensembles.",
    )
    ensemble_type: Optional[EnsembleType] = Field(
        None, description="Type of ensemble when applicable"
    )
    output_method: Optional[OutputMethod] = Field(
        None, description="Output method for ensemble when applicable"
    )
    ...
    def parse_ensemble_type(cls, values):
        ...
        if isinstance(type_value, str) and type_value.startswith("ensemble:"):
            ...
            if len(parts) != 3:
                raise ValueError(
                    "Invalid ensemble format. Must be 'ensemble:<ensemble_type>:<output_method>'"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid values for ensemble_type or output_method: {str(e)}"
                )
    ...
    def check_ensemble_fields(self):
        ...
        if self.type == ClassifierType.ensemble:
            if not self.ensemble_type or not self.output_method:
                raise ValueError(
                    "ensemble_type and output_method must be provided for ensemble type classifiers"
                )
        return self
    ...
    def check_img_size(self):
        ...
        if (
            self.type == ClassifierType.ensemble
            and self.ensemble_type == EnsembleType.pretrained
        ):
            # Pretrained models accept 1-channel images
            if img_size[0] != 1:
                raise ValueError(
                    "Pretrained models in ensemble expect 1-channel images (got {img_size[0]} channels)"
                )
        else:
    ...
    def check_nf(self):
        ...
        if self.type == ClassifierType.cnn:
            if not isinstance(self.nf, list) or any(
                not isinstance(n, int) for n in self.nf
            ):
                raise ValueError(
                    "For cnn type, nf must be a list of integers.")
            ...
            if self.ensemble_type == EnsembleType.cnn:
                if not isinstance(self.nf, list) or any(
                    not isinstance(n, list) for n in self.nf
                ):
                    raise ValueError(
                        "For ensemble with cnn type, nf must be a list of lists of integers."
                    )
            elif self.ensemble_type == EnsembleType.pretrained:
                if self.nf is not None:
                    raise ValueError(
                        "For ensemble with pretrained type, nf should be None."
                    )
        return self
...
class PoolingParams(BaseModel):
    ...
    in_size: int = Field(
        ..., description="Input size (height or width) for pooling operation."
    )
    kernel: int = Field(...,
                        description="Kernel size for pooling or convolution.")
    dilation: int = Field(
        1, description="Dilation factor for pooling or convolution.")
    ...
    def check_valid_values(self):
        ...
        if self.in_size < 0 or self.kernel < 0 or self.dilation < 0 or self.padding < 0:
            raise ValueError(
                "Input size, kernel, dilation, and padding must be non-negative."
            )
...
class EvaluateParams(BaseModel):
    ...
    device: DeviceType = Field(
        ..., description="Device for computation ('cpu', 'cuda')."
    )
    verbose: bool = Field(
        True, description="Whether to show progress bar or not.")
    desc: str = Field(
        "Validate", description="Description for the progress bar.")
    header: Optional[str] = Field(
        None, description="Optional header for logging.")


class DefaultTrainParams(BaseModel):
    """Pydantic model for default_train_fn parameters."""

    early_acc: float = Field(
        1.0, description="Early accuracy threshold for backpropagation."
    )
    device: DeviceType = Field(
        DeviceType.cuda, description="Device for computation ('cpu' or 'cuda')."
    )
...
class TrainArgs(BaseModel):
    ...
    early_stop: Optional[int] = Field(
        None, description="Early stopping criterion")
    early_acc: float = Field(1.0, description="Early accuracy threshold")
    out_dir: str = Field(...,
                         description="Output directory for saving checkpoints")
...
class TrainingStats(BaseModel):
    ...
    best_epoch: int = Field(
        0, description="Epoch with the lowest validation loss.")
    early_stop_tracker: int = Field(
        0, description="Counter for early stopping criteria."
    )
    cur_epoch: int = Field(0, description="Current epoch during training.")
    train_acc: List[float] = Field(
        default_factory=list, description="List of training accuracies for each epoch."
    )
    train_loss: List[float] = Field(
        default_factory=list, description="List of training losses for each epoch."
    )
    ...
    val_loss: List[float] = Field(
        default_factory=list, description="List of validation losses for each epoch."
    )
    test_acc: List[float] = Field(
        default_factory=list, description="List of test accuracies for each epoch."
    )
    test_loss: List[float] = Field(
        default_factory=list, description="List of test losses for each epoch."
    )
...
class CLTrainArgs(BaseModel):
    ...
    data_dir: str = Field(
        default=f"{os.environ['FILESDIR']}/data", description="Path to dataset"
    )
    out_dir: str = Field(
    ...
    name: Optional[str] = Field(
        default=None, description="Name of the classifier for output files"
    )
    dataset_name: str = Field(
        default="mnist", description="Dataset (mnist or fashion-mnist)"
    )
    pos_class: int = Field(
        default=7, description="Positive class for binary classification"
    )
    neg_class: int = Field(
        default=1, description="Negative class for binary classification"
    )
    ...
    early_stop: Optional[int] = Field(
        default=None, description="Early stopping criteria (optional)"
    )
    early_acc: float = Field(
        default=1.0, description="Early accuracy threshold for backpropagation"
    )
    lr: float = Field(
        default=5e-4, description="Learning rate for the optimizer")
    nf: str = Field(
        default="2", description="Number of filters or features in the model"
    )
