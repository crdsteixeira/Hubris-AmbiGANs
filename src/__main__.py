...
def gen_classifiers(config: ConfigMain, classifier: CLTrainArgs) -> None:
    ...
    args: list[str] = [
        ...
        "--early_stop",
        str(classifier.early_stop),
        "--early_acc",
        str(classifier.early_acc),
        "--lr",
        str(classifier.lr),
        "--nf",
        str(classifier.nf),
...
def main() -> None:
    ...
    if config.gen_gan:
        fid_stats_path = os.path.join(
            config.data_dir,
            "fid-stats",
            f"stats.inception.{config.dataset.name}.{config.dataset.binary.pos}v{config.dataset.binary.neg}.npz",
        )
