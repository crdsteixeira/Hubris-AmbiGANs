...
def create_and_store_z(out_dir, n, dim, name=None, config=None):
    ...
    with open(os.path.join(out_path, "z.npy".format()), "wb") as f:
        np.savez(f, z=noise)

    if config is not None:
        with open(os.path.join(out_path, "z.json".format()), "w") as out_json:
    ...
    if "," in args.nf:
        ...
        for n in l_nf:
            if "-" in n:
                ...
                cnn = [
                    np.random.randint(1, high=n)
                    for _ in range(np.random.randint(1, high=4 + 1))
                ]
        ...
        cnn = [
            [
                np.random.randint(1, high=5 + 1)
                for _ in range(np.random.randint(2, high=4 + 1))
            ]
